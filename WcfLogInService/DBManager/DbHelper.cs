...
namespace WcfLogInService
{
    public class DbHandler
    {
       ...
       // bool IsActive = false;
        ...
        public DbHandler(string ConnectionString, string Name, int commandTimeout = 10)
        {
        ...
        public SqlDataReader ExecuteQuery(SqlCommand cmnd, out string errMsg)
        {
            ...
            using (SqlConnection _conn = new SqlConnection(this.ConnectionString))
            {
                try
                {
                    _conn.Open();
                    ...
                    // for log issue
                    if (cmnd.CommandText.Length > 80)
                        cmnd.CommandText = cmnd.CommandText.Substring(0, 80);
        ...
        public int Executefunction(string  cmndFunc, string loginID,string errMsg)
        {
            try
            {
                errMsg = "";
                MccMLogger.Debug("  >DM::Executefunction. func = " + cmndFunc);
                SqlConnection _conn = new SqlConnection(this.ConnectionString);
                object funcRC;
                if (_conn.State == ConnectionState.Closed)
                    _conn.Open();
                MccMLogger.Debug("  >DM::Executefunction. func = " + cmndFunc);
                SqlCommand cmnd = new SqlCommand(cmndFunc, _conn);
                SqlParameter agentid = new SqlParameter("@AgentID", SqlDbType.VarChar);
                agentid.Value = loginID;
                cmnd.Parameters.Add(agentid);
                funcRC = cmnd.ExecuteScalar();
                return (int)funcRC;
/*
       ...
       // cmd.CommandType=CommandType.StoredProcedure;
        ...
        Response.Write(str.ToString());
            ...
            */
            ...
            {
                MccMLogger.Error("Executefunc:: Exception during executing command: " + cmndFunc);
                MccMLogger.Error("  ->Error: " + ex.Message);
                errMsg = ex.Message;
            }
            return 0;
        ...
        public ReturnData ExecQuery(SqlCommand cmnd)
        {
            string ErrMsg = "";
            this.ExternalOutput = "";
        ...
        public ReturnData ExecNonQuery(SqlCommand cmnd)
        {
            string ErrMsg = "";
            this.ExternalOutput = "";
            ReturnData rt = null;
        ...
        public bool ExecNonQuery(SqlCommand cmnd, out string ErrMsg)
        {
            ErrMsg = "";
            this.ExternalOutput = "";
        ...
        {
        MccMLogger.Error("OpenConnection:: Failed to open Connection: " + ConnectionName + ". Reason: " + ex.Message);
        MccMLogger.WriteEventLog("Failed to open connection " + ConnectionName +
        Environment.NewLine + ex.Message, "DB Manager");
        ConnectionOpen = false;
        ExternalOutput = "Failed to open Connection: " + ConnectionName + ". Reason: " + ex.Message;
        errMsg = ex.Message;
        }
        return retval;
        }*/
        ...
        /*public bool ExecuteNonQuery(SqlCommand cmnd, out int EffectedRecord, out string errMsg)
        {
        bool retVal = false;
        errMsg = "";
        EffectedRecord = 0;
        ...
        /*public bool VerifyConnectionStatus(out string errMsg)
        {
            bool retval = false;
            errMsg = "";
            //MccMLogger.Debug("Verify connection to DataBase status...");
            try
            {
            ...
            {
                MccMLogger.Error("Failed to open Connection: " + ConnectionName + ". Reason: " + ex.Message);
                errMsg = ex.Message;
            }
            return retval;
        }*/
